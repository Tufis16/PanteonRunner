Shader "Hidden/Universal Render Pipeline/CameraMotionBlur"
{
    HLSLINCLUDE
        ...
        TEXTURE2D_X(_MotionVectorTexture); //Custom object motion blur
...
#define _ViewProjM unity_MatrixVP
        ...
        half2 _Jitter; //Custom object motion blur
        ...
        half4 DoMotionBlur(VaryingsCMB input, int iterations)
        {
            ...
            //half2 velocity = GetCameraVelocity(float4(uv, input.uv.zw)) * _Intensity; Disabling camera motion blur
            half2 velocity = ClampVelocity(SAMPLE_TEXTURE2D_X(_MotionVectorTexture, sampler_LinearClamp, uv.xy).rg - _Jitter, _Clamp) * _Intensity; //Object Motion Blur
